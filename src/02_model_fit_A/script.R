#This script loads the Case data produced in task 01,
#filters it slightly to variables of interest
#runs a stan model,
...
close(fileConn)
################################################################################
#We output a lot of columns for Case_Rates_Data in task 01,
#filter to just the ones we're interested in immediately.
...
#Final week is an orderly parameter,
#by default, final_week is 96 - (w/b 27/02/22)
 #remove data after this date
...
#One for if we are opting to scale by the estimated proportion of the susceptible population
#And one where we don't.
#We have a different model string depending on which option we choose:
...
data {
  ...
  matrix<lower=0,upper=1>[N,T] susceptible_proxy; //Factor in an estimate of how many susceptibles in the population
...
data {
  ...
  matrix<lower=0,upper=1>[N,T] susceptible_proxy; //Factor in a metric of how many susceptibles in the population
...
for(i in 2:final_week){
  ...
  #We build the y and E matrices week by week
  if(cases_type == "Dashboard"){
    y[j,] = Reduced_Data$next_week_cases;
    #We don't allow a 0 value as this generates errors in the distr.
...
#Standardise our covariates so the beta values are comparable
...
#We need to re-calculate W for just the English LTLAs
...
#Plot LOO-CV criterions using moment-matching
if(output_loo){
